...
AMaze_Generation::AMaze_Generation()
{
	...
	TempFloor->SetupAttachment(RootComponent);
	
	TempWall = CreateDefaultSubobject<UHierarchicalInstancedStaticMeshComponent>(TEXT("TempWall"));
	TempWall->SetupAttachment(RootComponent);
}
...
void AMaze_Generation::OnConstruction(const FTransform& Transform)
{
	...
	TempWall->ClearInstances();
	WallTiles.Empty();
	for (int i = 0; i <= SizeOfMaze.X; i++)
	{
		for (int j = 0; j <= SizeOfMaze.Y; j++)
		{
			FIntPoint Temp = FIntPoint(i, j);
			if (PathTiles.Contains(Temp))
			{
				FVector TempLoc = FVector(Temp.X * TileSize, Temp.Y * TileSize, 0);
				TempFloor->AddInstance(FTransform(TempLoc));
			} else
			{
				FVector TempLoc = FVector(Temp.X * TileSize, Temp.Y * TileSize, 0);
				WallTiles.Emplace(Temp);
				TempWall->AddInstance(FTransform(TempLoc));
			}
		}
	}
...
FTransform AMaze_Generation::BuildTilesLoc(FIntPoint Location, float ZAxis, int RotationIndex)
{
	FVector LocLocation = FVector(Location.X * TileSize, Location.Y * TileSize, ZAxis);
	FRotator WallRotation;
	switch (RotationIndex)
	{
	case 0: WallRotation = FRotator(0, -90, 0);
		break;
	case 1: WallRotation = FRotator(0, 0, 0);
		break;
	case 2: WallRotation = FRotator(0, 90, 0);
		break;
	case 3: WallRotation = FRotator(0, 180, 0);
	}
	return UKismetMathLibrary::MakeTransform(LocLocation + GetActorLocation(), WallRotation, FVector(1, 1, 1));
}
...
void AMaze_Generation::SpawnMazeMeshes()
{
	FIntPoint LoopLocation;
	int LoopIndex;
	TArray<bool> SegmentIndex;
	for (int i = 0; i < SizeOfMaze.X;  i++)
	{
		for (int j = 0; j < SizeOfMaze.Y; j++)
		{
			LoopLocation.X = i;
			LoopLocation.Y = j;
			if (PathTiles.Contains(LoopLocation) && InstanceManager)
			{
				InstanceManager->AddNewMeshInstance(FloorMesh, BuildTilesLoc(LoopLocation, 0, 0));
				TArray<FIntPoint> Neighbours = GetNeightbourLocation(1, LoopLocation);
				for (int k = 0; k < Neighbours.Num() - 1; k++)
				{
					LoopIndex = k;
					SegmentIndex.Insert(PathTiles.Contains(Neighbours[k]), LoopIndex);
					if (!PathTiles.Contains(Neighbours[k]))
					{
						InstanceManager->AddNewMeshInstance(WallMesh, BuildTilesLoc(LoopLocation, 0, LoopIndex));
					}
				}
			}
		}
